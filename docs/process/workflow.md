...
- 本文件只記錄個人離線開發流程。
- 部署已就緒：push `main` 觸發 GitHub Actions（見 `AGENTS.md` §下次開工摘要）。
- 專案主線已結案；後續以維護或可選方向為主。
- 若流程涉及刪除、覆寫、重命名或不可逆 Git 操作，必須先列出影響清單並等待使用者確認。

## 每次開工流程

1. 讀 `AGENTS.md` §下次開工摘要 與 §已知問題。
2. 依 `AGENTS.md` §開工讀檔規則 讀取其他必要文件。
3. 確認本次需求屬維護、可選方向或新功能（主線已結案）。
4. 使用 Git 確認工作區狀態。
5. 只針對本次任務需要的檔案進行修改。
...
1. 確認需求範圍（見 AGENTS §後續可選方向）。
2. 若有多種實作方案，依 `AGENTS.md` §操作確認規則 使用選項彈窗。
3. 先完成最小可用版本。
4. 補上必要的型別、資料儲存與基本錯誤處理。
5. 手動測試桌面瀏覽器與手機尺寸；PWA 行為用 HTTPS 正式網址或本機 build。
6. 更新 `docs/process/Project_Log.md` 實作紀錄。
7. 若階段任務狀態改變，同步更新 `AGENTS.md`；若產品規格改變，同步更新 `docs/product/Spec.md` 與 `docs/README.md`。
